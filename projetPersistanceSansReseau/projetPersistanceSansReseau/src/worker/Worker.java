...
public class Worker extends Thread {
		...
		private void persistence( final BigInteger MIN ,final BigInteger MAX ){
			...
			while( iterations.subtract(offSet).compareTo(BigInteger.ZERO) == 1 ) {  
				...
				while( nb.length() > 1 ){                            // la taille du nombre intermediaire diminue : on boucle tant que taille > 1 
					
					for( int j = 0 ; j < nb.length()  ; j++ ) {      // on parcours tout le nombre nb
						//nbInter = nbInter * ( nb.charAt(i) ) ; 
						nbInter = nbInter.multiply( BigInteger.valueOf( Character.getNumericValue( nb.charAt( j ) ) ) );
					}
					
					nb = nbInter.toString();                         // on met le nombre a calculer a la valeur intermediaire
					nbInter = BigInteger.ONE;                        // reinitialisation du nombre intermediaire
					persistance++;                                   // incrementation de la persistance de ce nombre
					
				}
										
				this.list.add( nbInt.toString() + ":" + persistance );
				
				nbInt = nbInt.add( new BigInteger("1") );			 // on incremente le nombre de base
				nb = nbInt.toString();                               // on met dans nb la conversion de nbInt en String
				persistance = 0;                                     // on reset la persistance
				
				iterations = iterations.subtract(BigInteger.valueOf(1));  // on decremente le nombre d'iterations restantes
			}
